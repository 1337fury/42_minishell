...
/*   Updated: 2023/06/06 15:57:43 by abdeel-o         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Creates a new command structure based on the current node and its
// subsequent nodes
...
// Builds a command row structure based on the current node and
...
t_row	*build_command(t_node **node)
{
	...
	family = family_init();
	position = 0;
	while (*node && (!((*node)->type % 3)
			|| !((*node)->type % 2) || (*node)->type == PIPE))
	{
