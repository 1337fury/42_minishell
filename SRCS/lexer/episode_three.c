...
/**
 * Handles operators in the given string, including logical operators (|| and &&),
...
void	operator_handler(t_list *lexer,  t_scanner *point, char *cmd)
{
	...
	{
		type = (c[0] == '|') * PIPE + (c[0] == '>') * GREAT + (c[0] == '<') * LESS;
...
/**
 * Handles default cases in the given string, where characters are not special characters or operators.
 * Advances the current position until a special character, operator, or whitespace is encountered.
 * Creates a new node in the lexer list with the extracted value, indicating whether it contains a wildcard or not.
 */
void	default_handler(t_list *lexer, t_scanner *point, char *cmd)
{
	...
	wildcard = false;
	while (cmd[point->current] && !ft_strchr("\'\"$()<|&>", cmd[point->current]) && !is_space(cmd[point->current]))
	{
